...
✅ **股票管理**
- 查看股票列表：实时获取股价，详细展示今日涨幅、当日盈亏、总盈亏、盈亏率和市值
- 支持添加/删除股票到投资组合
- 修改股票：通过交互界面修改已有股票的成本价和持仓数量
...
- 完美支持中文字符宽度对齐
- 智能颜色显示：红色盈利/上涨，绿色亏损/下跌，白色持平（0值）
- 股票间分隔线，提升可读性
...
**主菜单选项：**
- `► 股票列表` - 进入股票监控界面（实时显示价格、盈亏等详细数据）
- `  添加股票` - 添加新股票到投资组合
- `  修改股票` - 修改已有股票的成本价和持仓数量
- `  删除股票` - 删除投资组合中的股票  
- `  调试模式: 关闭` - 切换调试信息显示
- `  退出` - 保存并退出程序

**股票监控界面：**
1. 程序启动时如有股票数据会自动进入，或在主菜单选择"股票列表"
2. 系统每5秒自动更新股价数据，实时显示13列详细信息
3. 显示内容：股票代码、股票名称、现价、开盘、最高、最低、成本价、持股数、今日涨幅、当日盈亏、总盈亏、盈亏率、市值
4. 智能颜色显示：红色盈利/上涨，绿色亏损/下跌，白色持平
5. 显示投资组合汇总数据：总当日盈亏、总盈亏、总盈亏率、总市值
6. 按 ESC、Q键或M键返回主菜单
...
**修改股票：**
1. 使用方向键选择"修改股票"并回车确认
2. 使用方向键选择要修改的股票（显示当前成本价和数量）
3. 按回车确认选择，进入成本价修改界面
4. 实时输入新的成本价，按回车确认
5. 实时输入新的持仓数量，按回车确认完成修改
6. 系统自动保存修改并返回主菜单
7. 按 ESC 或 Q 可随时返回主菜单
...
- 红色：盈利/上涨数值（> 0.001）
- 绿色：亏损/下跌数值（< -0.001）
- 白色：持平数值（≈ 0），如今日涨幅0%、当日盈亏0元等
- 今日涨幅基于昨收价计算：(当前价-昨收价)/昨收价×100%
...
- **颜色显示逻辑**：红色表示盈利/上涨，绿色表示亏损/下跌，白色表示持平（0值）
...
**v2.5 - 界面逻辑优化**
- ✅ 智能启动：有股票数据时自动进入监控模式，提升用户体验
- ✅ 界面整合：主菜单"股票列表"直接进入监控界面，简化操作流程
- ✅ 增强导航：监控界面支持ESC、Q键、M键多种方式返回主菜单
- ✅ 移除冗余：精简菜单选项，删除重复的"查看股票列表"功能
- ✅ 消息清理：修复操作成功消息长期显示的问题
