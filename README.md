...
Stock Monitor 是一款专业的命令行股票监控工具，为投资者提供实时股价追踪、投资组合管理和资金流向分析功能。

### 🎯 核心特色

- **🔄 实时监控** - 5秒间隔自动刷新股价数据，掌握每个交易时刻
- **📊 专业分析** - 18列详细数据展示，包含开高低收、今日盈亏、持仓盈亏、盈亏率分析、市值统计
- **💰 资金流向** - 集成AKShare获取主力净流入、超大单、大单、中单、小单等6维度资金数据
- **🎨 智能显示** - 专业股票软件标准配色：红涨绿跌白平，数据一目了然
- **💼 投资组合** - 完整的持仓管理，支持添加、修改、删除股票操作
- **⭐ 自选股票** - 独立的股票关注列表，支持标签分组和资金流向分析
...
- **成本追踪**: 记录购买成本价和持股数量
- **自动计算**: 实时计算市值、盈亏等关键指标
- **排序功能**: 支持按11个字段进行升序/降序排序

### ⭐ 自选股票功能
- **关注列表**: 独立的股票关注列表，无需投入资金
- **实时刷新**: 每5秒自动更新自选股票的实时行情数据
- **标签管理**: 支持为股票添加自定义标签，按标签分组查看
- **资金流向**: 集成AKShare获取A股实时资金流向数据
...
- **Go 版本**: 1.25.0 或更高版本
- **Python环境**: Python 3.x（用于资金流向数据获取）
- **网络连接**: 用于获取实时股票数据
...
- **资金流向**: 集成AKShare获取实时资金流向数据，支持主力净流入等6个维度
...
**自选列表**（18列数据，含资金流向）：
```
标签 | 代码 | 名称 | 现价 | 今日涨幅 | 换手率 | 成交量 | 主力净流入 | 超大单 | 大单 | 中单 | 小单 | 净流入占比 | 主动买入 | 主动卖出 | 开盘 | 最高 | 最低
```
...
系统集成AKShare获取A股实时资金流向数据，支持以下6个维度：
...
- **智能缓存**: 避免频繁API请求
- **智能单位**: 自动选择万元/亿元单位显示
- **颜色指示**: 正值（流入/红色）、负值（流出/绿色）、零值（平衡/白色）
...
| 🥉 **3** | **AKShare API** | 🇨🇳 A股资金流向 | 6维度资金数据 | 实时 |
...
│
├── scripts/
│   └── akshare_fund_flow.py    # 🐍 AKShare资金流向数据获取脚本
│
├── venv/                       # 🐍 Python虚拟环境（资金流向功能）
│
├── doc/                        # 📖 文档目录
│   ├── FUND_FLOW_FIELDS.md     # 资金流向字段说明
│   ├── FUND_FLOW_README.md     # 资金流向功能使用说明
│   └── test_search_with_fund_flow.md # 搜索功能测试文档
│
├── README.md                   # 📝 中文文档
└── README_EN.md                # 📝 英文文档
```
...
| **AKShare** | `latest` | 资金流向数据 | • 实时资金流向<br>• 6维度数据<br>• Python生态 |
...
| 🐍 资金流向数据为0 | • Python环境问题<br>• AKShare库缺失 | • 安装Python依赖<br>• 检查虚拟环境 |
...
本版本重点进行了代码架构优化和国际化系统增强：
- 🏗️ **模块化重构**: 将系统常量迁移至 `consts.go`，国际化文本迁移至 `i18n/` 目录
- 🌐 **完整 i18n 支持**: 基于 JSON 配置文件的完整多语言体系
- 🎯 **用户体验优化**: 自选股票排序优化，新添加股票自动显示在顶部
- 🔧 **技术架构提升**: 精简主程序代码，提升可维护性和开发效率
...
- **[当前版本 v4.7](doc/version/v4.7.md)** - 架构优化与国际化增强 ⭐
...
- **标签分组**: 为自选股票添加标签（如"科技"、"医药"），便于分类查看
- **排序分析**: 利用排序功能按盈亏率、涨跌幅等维度分析股票表现
- **资金流向**: 关注A股的资金流向数据，判断主力资金动向
